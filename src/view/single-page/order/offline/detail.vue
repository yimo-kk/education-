    ...
    <div class="cj">
      <div class="tables">
        <ul>
          ...
          <li>
            <div>券码：<span>{{order.voucher}}</span></div>
          </li>
        </ul>
      </div>
      <div class="tables">
...
import { getOfflineCourseOrderById } from '@/api/courseOrder'
...
export default {
  ...
  methods: {
    ...
    handleOrder () {
      this.spinShow = true
      getOfflineCourseOrderById(this.$route.query.id).then((res) => {
        const data = res.data.data
        this.order = data.courseOrder
        this.nickName = data.nickName
        this.couponName = data.couponName
        this.courseList = data.educationOfflineCourseTerm
